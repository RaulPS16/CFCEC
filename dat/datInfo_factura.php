<?php
	...
	class datInfo_factura 
	{
		...
		*/
		...
		{
			try {
				$sql = "INSERT INTO info_factura VALUES (" . $pValores["posicion"] . ",'" . $pValores["descripcion"] . "');";
		...
		*/
		...
		{
			try {
				$sql = "UPDATE info_factura SET posicion = " . $pValores["posicion"] . ", descripcion = '" . $pValores["descripcion"] . "';";
				$this->dbm->ejecutar($sql);
			} catch (Exception $e) {
				// Carga el vector para hacer el reporte del error
				$this->datosBitacora = array('descripcion_error' => $e->getMessage() ,'error_num' => 1, 'modulo' => $pValores["modulo"], 'funcion' => __METHOD__, 'script_sql' => $sql, 'datos_pantalla' => IMPLODE(", ",$pValores));
		...
		*/
		...
		{
			try {
				$sql = "DELETE FROM info_factura WHERE posicion = " . $pValores["posicion"] . ";";
				$this->dbm->ejecutar($sql);
			} catch (Exception $e) {
				// Carga el vector para hacer el reporte del error
				$this->datosBitacora = array('descripcion_error' => $e->getMessage() ,'error_num' => 1, 'modulo' => $pValores["modulo"], 'funcion' => __METHOD__, 'script_sql' => $sql, 'datos_pantalla' => IMPLODE(", ",$pValores));
		...
		*/
		...
		{
			try {
				$sql = "SELECT * FROM info_factura WHERE posicion = " . $pValores["posicion"] . ";";
